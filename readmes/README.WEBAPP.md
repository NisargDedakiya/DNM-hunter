# NisargHunter AI Web Application
...
docker build -t nisarghunter-webapp:latest .

# Run production container locally
docker run -p 3000:3000 \
  --network graph_db_default \
  -e NEO4J_URI=bolt://nisarghunter-neo4j:7687 \
  -e NEO4J_USER=neo4j \
  -e NEO4J_PASSWORD=your_password \
  nisarghunter-webapp:latest
```
...
# Tag image
docker tag nisarghunter-webapp:latest \
  YOUR_ACCOUNT_ID.dkr.ecr.us-east-1.amazonaws.com/nisarghunter-webapp:latest
...
```json
{
  ...
  "containerDefinitions": [
    {
      "name": "webapp",
      "image": "YOUR_ACCOUNT.dkr.ecr.us-east-1.amazonaws.com/nisarghunter-webapp:latest",
      "portMappings": [
        {
          "containerPort": 3000,
          "protocol": "tcp"
      ...
      "secrets": [
        {
          "name": "NEO4J_PASSWORD",
          "valueFrom": "arn:aws:secretsmanager:us-east-1:YOUR_ACCOUNT:secret:nisarghunter/neo4j"
        }
      ],
      "healthCheck": {
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/nisarghunter-webapp",
...
aws ecs create-service \
  ...
  --service-name nisarghunter-webapp \
  --task-definition nisarghunter-webapp \
  --desired-count 2 \
  --launch-type FARGATE \
...
aws application-autoscaling register-scalable-target \
  ...
  --resource-id service/your-cluster/nisarghunter-webapp \
  --min-capacity 2 \
  --max-capacity 100
...
|----------|-------------|---------|
| `NEO4J_URI` | Neo4j connection URI | `bolt://localhost:7687` (dev) / `bolt://nisarghunter-neo4j:7687` (Docker) |
| `NEO4J_USER` | Neo4j username | `neo4j` |
| `NEO4J_PASSWORD` | Neo4j password | (set in .env.local) |
| `DATABASE_URL` | PostgreSQL connection string | `postgresql://nisarghunter:<POSTGRES_PASSWORD>@localhost:5432/nisarghunter` (password from `.env`, generated on fresh install; host port loopback-only) |
...
The `AIAssistantDrawer` is the primary interface for interacting with the NisargHunter AI pentesting agent. It communicates over WebSocket via the `useAgentWebSocket` hook and renders a real-time timeline of agent activity.
