...
docker network create nisarghunter-network
...
| `RECON_IMAGE` | `nisarghunter-recon:latest` | Docker image for recon |
...
services:
  recon-orchestrator:
    build: .
    container_name: nisarghunter-recon-orchestrator
    ports:
    ...
    environment:
      ...
      - RECON_IMAGE=nisarghunter-recon:latest
      ...
      - LOCAL_LLM_NETWORK=nisarghunter-orchestrator-net
    networks:
      - orchestrator-net

networks:
  # Network isolation: the privileged orchestrator (Docker-socket holder) lives
  # on its OWN network, NOT on `nisarghunter`. Only the webapp (multi-homed) and the
  # on-demand Ollama judge share it, so a compromised worker cannot reach the
  # orchestration API.
  orchestrator-net:
    name: nisarghunter-orchestrator-net
    external: true
```
...
> it sits on `nisarghunter-orchestrator-net` (shared only with the webapp and the Ollama
> judge) rather than the shared `nisarghunter` network, and its host port is bound to
...
2. Creates a new container with:
   ...
   - Filtering broker socket for nested (sibling) container execution, restricted
     to the known tool images. It is served on the `nisarghunter_broker_socket` named
...
1. Check Docker socket is accessible:
   ```bash
   docker exec nisarghunter-recon-orchestrator docker ps
   ```

2. Verify recon image exists:
   ```bash
   docker images | grep nisarghunter-recon
   ```

3. Check orchestrator logs:
   ```bash
   docker logs nisarghunter-recon-orchestrator
   ```
...
2. Check container is running:
   ```bash
   docker ps | grep nisarghunter-recon
   ```

### Connection Refused from Webapp

The orchestrator is **not** on the shared `nisarghunter` network — it lives on
`nisarghunter-orchestrator-net`, and the webapp is multi-homed onto that network to
reach it. If the webapp cannot reach `recon-orchestrator:8010`, confirm the webapp
is attached to `nisarghunter-orchestrator-net` (it must be on both `nisarghunter` and
`nisarghunter-orchestrator-net`). The host-published port is bound to `127.0.0.1`, so
