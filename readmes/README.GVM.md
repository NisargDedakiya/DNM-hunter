...
### NisargHunter AI Integration

NisargHunter AI uses GVM in **headless API mode** (no web GUI) to:
...
**Architecture:** The scan flow mirrors the recon pipeline: Webapp API → Recon Orchestrator → Docker container (`nisarghunter-vuln-scanner`) → SSE log streaming → graph update.
...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                              NisargHunter AI GVM Architecture                               │
├─────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                     │
│   ┌─────────────────┐                                                               │
...
│   ┌─────────────────┐     Unix Socket      ┌─────────────────┐                      │
│   │  PYTHON SCANNER │────────────────────▶│      GVMD       │                      │
│   │   (nisarghunter-     │   GMP Protocol       │   (Manager)     │                      │
...
#### 1. **nisarghunter-gvm-gvmd** (Greenbone Vulnerability Manager Daemon)
...
#### 2. **nisarghunter-gvm-ospd** (OSPd-OpenVAS Scanner Daemon)
...
#### 3. **nisarghunter-gvm-postgres** (PostgreSQL Database)
...
#### 4. **nisarghunter-gvm-redis** (Redis Cache)
...
#### 5. **nisarghunter-gvm-notus-scanner** (Notus Scanner)
...
NisargHunter AI uses **data containers** that download feeds once and populate Docker volumes:
...
│   ├── Dockerfile                    # Python scanner image (nisarghunter-vuln-scanner)
