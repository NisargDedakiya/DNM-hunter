...
export const MCP_PRESETS: McpPreset[] = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Research vulnerable libraries, understand exploit code in public PoC repos, audit dependencies before targeting them.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Find exploit PoCs, search for credentials in public repos, identify vulnerable code patterns, supply-chain analysis.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Find security research papers (e.g., recent CVE writeups, exploitation techniques), datasets for fuzzing, ML-based vuln-detection tools.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Look up current API surface of libraries you\'re probing (e.g., to understand authentication flows or find deprecated/removed protections).',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Independent OSINT search engine — useful when Tavily/Google rate limits hit during recon, or for cross-checking results from a different index.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Scrape target pages, fetch CVE advisories, pull from blog posts/changelogs/release notes — agent-side equivalent of curl with content extraction.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Carry findings across sessions: track confirmed credentials, persistent shells, mapped subdomain → CVE relationships beyond a single chat.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Force the LLM to plan exploit chains explicitly: enumerate steps, revise hypothesis, document decision points. Audit-trail-friendly.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Stash exfiltrated files, build wordlists/payloads incrementally, save reverse-shell artifacts between iterations. Sandboxed to /app/logs/sandbox.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Correlate event timestamps across logs, scheduling, time-zone-aware credential expiry checks. Best first preset to verify the integration works end-to-end.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Map external attack surface, find exposed services, banner-grab without touching the target. Augments execute_naabu by adding pre-existing scan data + CVE matches.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Triage suspicious artifacts found during scans, check IP reputation before exfil callbacks, identify known-malicious domains hosted on target subdomains.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Single-call DNS recon (lookups, reverse, SPF chain, DMARC, wildcards, SRV), CT log search (crt.sh), passive ASN/BGP analysis. Fills gaps the kali-sandbox tools don\'t cover natively.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'IP reputation enrichment — distinguish opportunistic scanners from targeted attackers, validate exfil destinations, check WAF blocklists. Multiple feeds cross-corroborate.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'When you exfil source or have access to a target\'s public GitHub: scan it for SQLi, XSS, SSRF, hard-coded secrets, deserialization gadgets. Custom-rule support for one-off pattern hunts.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Backup for built-in web_search if Tavily key isn\'t set globally; also exposes extract/crawl tools the built-in lacks. Useful when scoping a target — fetch their entire site.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Find writeups by description ("recent CVE-2024 RCEs in Java deserializers"), discover similar PoCs to a known exploit, semantic search for vendor-specific advisories.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Stealth-mode OSINT search that doesn\'t correlate with your project\'s Tavily/Brave keys. Useful when you want zero attribution to your infra during attribution-sensitive recon.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Once SQLi grants DB access, point this at the captured DSN to enumerate schemas/tables/users/permissions structurally — much cleaner than crafting SQL through the injection point.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Independent browser channel: run a parallel exploit with Puppeteer while system execute_playwright handles the primary session. Useful for SAML/OAuth flows where two browser contexts are needed.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Notify your team Slack when the agent confirms a vuln, completes a phase, or needs human approval. Also useful as a low-noise output channel during long unattended scans.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Background on a target organization, vendor history, biographical info on email enumeration targets. Neutral source independent of your search-API rate limits.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Drives ZAP\'s active scanner from the agent — automated XSS/SQLi/SSRF/path-traversal hunts on web targets, with payload variation that nuclei templates don\'t cover.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'When you obtain leaked AWS credentials (e.g., via execute_gau finding env files, GitHub PAT exfil), enumerate S3 buckets, IAM users/roles, DynamoDB tables, and identify privilege-escalation paths.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Independent corroboration of Shodan data; Censys often catches assets Shodan misses. Strong on TLS/cert intel — find shadow IT subdomains via SAN-list mining.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Phishing simulation prep, social-engineering target enumeration, AD username-pattern discovery (often emails == usernames). Free tier 25 req/month.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Check whether emails captured during recon appear in known breaches → predict valid passwords for credential stuffing. Domain-wide search reveals breach exposure for the entire target.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Active web-app pentest beyond what curl can do — capture mobile-app / SPA traffic, manipulate JWT tokens mid-flight, replay session-fixation payloads. Complements execute_playwright.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Triage exfiltrated SQLite files (mobile-app DBs, Chrome cookies/history, Slack desktop, Signal, password managers). Many high-value artifacts ship as .db files.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Stream findings into a structured engagement workspace as the agent confirms vulns. Pages can become draft sections of the final pentest report. 22 tools.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'When the local Playwright/Puppeteer hits aggressive bot detection (Cloudflare Turnstile, PerimeterX, Akamai), Browserbase provides residential-IP-backed sessions with anti-detection profiles.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'When a target uses Kubernetes and you obtain a kubeconfig (leaked, RBAC misconfig, privileged pod escape), enumerate namespaces, secrets, service accounts, and look for privilege-escalation paths.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Scan target source you\'ve exfiltrated or have access to (open-source dependencies, package.json, requirements.txt, etc.) for known CVEs and license risks.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'When pentesting an e-commerce target that uses Stripe, you can validate the merchant\'s test-mode flow, simulate fraudulent card patterns, and check webhook signature verification with safe test keys.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Convert each confirmed vulnerability into a Linear issue automatically (severity, repro steps, evidence). Sync with the client\'s remediation team during long engagements.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Scan target Docker images / Kubernetes manifests / Terraform after exfil — identifies vulnerable base images, hard-coded secrets, IaC misconfig (open security groups, public S3, etc).',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'When you obtain cloud credentials, runs 300+ checks against the target tenancy: IAM mistakes, public S3, weak RDS encryption, open NSGs, etc. Catches things the basic AWS MCP can\'t.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'Deeper CVE context than the built-in cve_intel tool: EPSS exploitation probability, CISA known-exploited status, ATT&CK technique mappings, OSV cross-reference. Useful for prioritizing exploit targets.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForNisargHunter: 'When you exfiltrate proprietary binaries (Linux ELF, Windows PE, mobile-app native libs), this lets the LLM disassemble / decompile / search strings + xrefs to find auth bypasses, hard-coded secrets, vulns.',
